...
import { REFERRAL_LEVELS, MAX_REFERRAL_COMMISSION_PERCENTAGE } from "../models/Referral"
import { MAX_ROI_PERCENTAGE, MAX_COMMISSION_PERCENTAGE } from "../models/Investment"

/**
 * Calculate daily ROI for an investment
 * Returns the ROI amount or null if investment has reached limits
 */
async function calculateDailyROI(investment: any): Promise<number | null> {
  const maxRoiEarnings = (investment.amount * MAX_ROI_PERCENTAGE) / 100
 ...
 */
async function hasActiveReferrals(userId: string): Promise<boolean> {
...
async function distributeReferralCommissions(
  ...
  const investor = await UserModel.findById(investment.userId)
  if (!investor) return
  ...
  for (const chainLink of referralChain) {
    if (totalCommissionPercentage >= MAX_REFERRAL_COMMISSION_PERCENTAGE) break

    const levelConfig = REFERRAL_LEVELS.find((l) => l.level === chainLink.level)
    if (!levelConfig) continue

    const commissionPercentage = Math.min(
      levelConfig.percentage,
      MAX_REFERRAL_COMMISSION_PERCENTAGE - totalCommissionPercentage
    )
    const commissionAmount = (roiAmount * commissionPercentage) / 100

    // Update referrer's balance directly
    const referrer = await UserModel.findById(chainLink.userId)
    if (referrer) {
      ...
      const referrerInvestments = await InvestmentModel.find({
        userId: new mongoose.Types.ObjectId(chainLink.userId),
        status: "active",
      })
      ...
      if (referrerInvestments.length > 0) {
        ...
        for (const refInvestment of referrerInvestments) {
          const maxCommissionEarnings = (refInvestment.amount * MAX_COMMISSION_PERCENTAGE) / 100
      ...
      const referral = await ReferralModel.findOne({
        referrerId: new mongoose.Types.ObjectId(chainLink.userId),
 ...
 */
export async function processDailyROI(): Promise<void> {
  try {
    ...
    for (const investment of activeInvestments) {
      try {
        ...
        if (roiAmount === null || roiAmount <= 0) {
          ...
          const hasNetworking = await hasActiveReferrals((investment.userId as any)._id.toString())
          const maxEarnings = hasNetworking
            ? (investment.amount * MAX_COMMISSION_PERCENTAGE) / 100
            : (investment.amount * MAX_ROI_PERCENTAGE) / 100
        ...
        // Update user balance
        const user = await UserModel.findById((investment.userId as any)._id)
        if (user) {
          ...
          await TransactionModel.create({
            userId: (investment.userId as any)._id,
            type: "roi",
            amount: roiAmount,
            status: "completed",
          ...
          if (investment.totalReturns >= maxEarnings) {
            ...
            expiredCount++
            console.log(
              `✅ Investment ${investment._id} expired (reached ${hasNetworking ? "400%" : "300%"} limit)`
            )
          }
    ...
    console.log(
      `✅ Daily ROI processing complete: ${processedCount} processed, ${expiredCount} expired`
    )
  } catch (error) {
