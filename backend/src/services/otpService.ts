...
export async function sendOTP(
  email: string,
  options?: { subject?: string; title?: string; purpose?: string }
  ...
  try {
    ...
    if (!gmailUser || !gmailPassword) {
      ...
      if (process.env.NODE_ENV === "development") {
        console.warn(
          "⚠️  Gmail credentials not configured. OTP generated but email not sent."
        )
    ...
    if (error.code === "EAUTH" && process.env.NODE_ENV === "development") {
      ...
      if (stored) {
        console.warn(
          "⚠️  Email authentication failed. OTP generated but email not sent."
        )
